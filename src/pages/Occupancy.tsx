...
export function Occupancy() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-lg font-bold text-brand-charcoal">Occupancy</h1>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-brand-stone/20">
                  {data.map((row) => (
                    <tr key={row.arrival_date} className="hover:bg-brand-blanco transition-colors">
                      <td className="px-4 py-3 text-brand-charcoal/80 whitespace-nowrap">{fmtDate(row.arrival_date)}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal/80">{row.num_arrivals?.toLocaleString() ?? '—'}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal/80">{row.total_guests?.toLocaleString() ?? '—'}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal font-medium whitespace-nowrap">{fmtCurrency(row.room_revenue)}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal/80 whitespace-nowrap">{fmtCurrency(row.folio_pos_revenue)}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal/80 whitespace-nowrap">{fmtCurrency(row.pos_outlet_revenue)}</td>
                      <td className="px-4 py-3 tabular-nums text-brand-charcoal/80 whitespace-nowrap">{fmtCurrency(row.pos_rev_per_guest)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
